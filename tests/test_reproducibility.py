...
@pytest.mark.contract
@pytest.mark.asyncio
async def test_wrong_tool_result_misses(tmp_path: Path) -> None:
    session = claude_tool_session()
    cass = tmp_path / "miss"
    await record_session(session, cass)
    bad = deepcopy(session)
    bad.turns[1].request["body"]["messages"][2]["content"][0]["content"] = "WRONG"
    scrubber = Scrubber(hmac_key=b"parity")
    app = create_app(mode="replay", cassette_dir=cass, scrubber=scrubber)
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url="http://proxy") as client:
        ...
        r0 = await client.post(
            "/v1/messages",
            json=bad.turns[0].request["body"],
        )
        assert r0.status_code == 200
        r1 = await client.post(
            "/v1/messages",
            json=bad.turns[1].request["body"],
        )
